...
class embedder:
    ...
    def evaluate(self, batch_data, st):
        ...
        # Classifier Accuracy
        self.model_gnn.eval()
        _, preds = self.model_gnn.cls(batch_data)
    ...
    def summary(self):
        ...
        # 计算训练集最大准确率
        train_acc_max = torch.tensor(self.train_accs).max().item()

        # 计算验证集最大准确率
        val_acc_max = torch.tensor(self.valid_accs).max().item()

        # 计算测试集最大准确率
        test_acc_max = torch.tensor(self.test_accs).max().item()

        # 打印最大准确率
        print("** | train max acc : {:.2f} | valid max acc : {:.2f} | test max acc : {:.2f} |  **\n".format(
            train_acc_max, val_acc_max, test_acc_max))
